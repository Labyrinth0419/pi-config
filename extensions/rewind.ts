...
type RewindRetentionSettings = NonNullable<
  NonNullable<RewindSettings["rewind"]>["retention"]
>;
...
function getSettings(): RewindSettings {
  ...
  try {
    cachedSettings = JSON.parse(
      readFileSync(getSettingsFilePath(), "utf-8"),
    ) as RewindSettings;
  } catch {
...
function getRetentionScanMode(): "ancestor-only" | "repo-sessions" {
  return getRetentionSettings()?.scanMode === "repo-sessions"
    ? "repo-sessions"
    : "ancestor-only";
}
...
function isRewindForkPendingData(
  value: unknown,
): value is RewindForkPendingData {
  if (!value || typeof value !== "object") return false;
  const data = value as Partial<RewindForkPendingData>;
  return (
    data.v === 2 && typeof data.current === "string" && data.current.length > 0
  );
}
...
function addReferences(
  target: ParsedLedgerReference[],
  snapshots: string[],
  timestamp: number,
  data: RewindTurnData | RewindOpData,
) {
...
function resolveBindingSnapshotIndex(
  snapshots: string[],
  commitSha: string,
): number {
...
function addBindingToCollector(
  collector: ActivePromptCollector,
  entryId: string,
  commitSha: string,
) {
  const snapshotIndex = resolveBindingSnapshotIndex(
    collector.snapshots,
    commitSha,
  );
...
function getCommitFromData(
  data: RewindOpData,
  indexKey: "current" | "undo",
): string | undefined {
  const snapshotIndex = data[indexKey];
  return typeof snapshotIndex === "number"
...
function findLatestUserMessageEntry(
...
function findAssistantEntryForTurn(
  entries: SessionLikeEntry[],
...
export default function rewindExtension(pi: ExtensionAPI) {
  ...
  const parsedSessionCache = new Map<
    string,
    { mtimeMs: number; ledger: ParsedSessionLedger }
  >();
  ...
  function updateStatus(ctx: ExtensionContext) {
    ...
    ctx.ui.setStatus(
      ...
      theme.fg("dim", "◆ ") +
        theme.fg(
          "muted",
          `${entryToCommit.size} points / ${uniqueSnapshots} snapshots`,
        ),
    );
  }
  ...
  async function execGitChecked(args: string[]): Promise<GitExecResult> {
    ...
    if (result.code !== 0) {
      ...
      throw new Error(
        stderr || `git ${args.join(" ")} failed with code ${result.code}`,
      );
    }
    return result;
  }
  ...
  async function getRepoRoot(exec: ExecFn): Promise<string> {
    ...
    if (result.code !== 0) {
      ...
      throw new Error(
        stderr ||
          `git rev-parse --show-toplevel failed with code ${result.code}`,
      );
    }
  ...
  async function createStoreKeepaliveCommit(
  ...
  async function appendSnapshotToStore(commitSha: string): Promise<void> {
    ...
    while (attempts < 5) {
      ...
      const keepaliveCommit = await createStoreKeepaliveCommit(
        commitSha,
        oldHead,
      );
  ...
  async function rewriteStoreToLiveSet(
    liveCommitShas: string[],
  ): Promise<"rewritten" | "preserved-empty"> {
  ...
  async function deletePathsFromWorkingTree(paths: string[]) {
    ...
    for (const repoRelativePath of paths) {
      ...
      if (!isInsidePath(absolutePath, root)) {
        throw new Error(
          `refusing to delete path outside repo root: ${repoRelativePath}`,
        );
      }
  ...
  async function getDeletedPaths(
    currentTreeSha: string,
  ...
  function bindPendingPromptUser(
    entries: SessionLikeEntry[],
    collector: ActivePromptCollector,
  ) {
    ...
    const userEntry =
      findLatestMatchingUserMessageEntry(entries, collector.promptText) ??
      findLatestUserMessageEntry(entries);
    if (!userEntry) return;
    ...
    addBindingToCollector(
      collector,
      userEntry.id,
      collector.pendingUserCommitSha,
    );
    collector.pendingUserCommitSha = undefined;
  }

  function appendRewindTurn(
    ctx: ExtensionContext,
    collector: ActivePromptCollector,
  ) {
    ...
    if (latestBinding) {
      ...
      activeBranchState.currentTreeSha =
        lastExact?.commitSha === activeBranchState.currentCommitSha
          ? lastExact.treeSha
          : undefined;
    }
  ...
  function appendRewindOp(ctx: ExtensionContext, data: RewindOpData) {
    ...
    if (currentCommitSha) {
      ...
      activeBranchState.currentTreeSha =
        lastExact?.commitSha === currentCommitSha
          ? lastExact.treeSha
          : undefined;
    }
  ...
  function buildCurrentSessionLedger(
    ctx: ExtensionContext,
  ): ParsedSessionLedger {
    ...
    for (const rawEntry of ctx.sessionManager.getEntries() as SessionLikeEntry[]) {
      if (
        ...
        rawEntry.customType === "rewind-turn" &&
        isRewindTurnData(rawEntry.data)
      ) {
        applyBindings(
          ledger.entryToCommit,
          rawEntry.data.snapshots,
          rawEntry.data.bindings,
        );
        addReferences(
          ledger.references,
          rawEntry.data.snapshots,
      ...
      if (
        rawEntry.type === "custom" &&
        rawEntry.customType === "rewind-op" &&
        isRewindOpData(rawEntry.data)
      ) {
        applyBindings(
          ledger.entryToCommit,
          rawEntry.data.snapshots,
      ...
      if (
        rawEntry.type === "custom" &&
        rawEntry.customType === "rewind-fork-pending" &&
        isRewindForkPendingData(rawEntry.data)
      ) {
  ...
  async function parseSessionLedgerFile(
    sessionFile: string,
    ...
    try {
      ...
      if (!hasRewindEntries) {
        ...
        for (let i = 0; i < 5 && pos < content.length; i++) {
          ...
          const line =
            nextNewline >= 0
              ? content.substring(pos, nextNewline)
        ...
        parsedSessionCache.set(sessionFile, {
      ...
      for (const line of lines) {
        ...
        if (
          entry?.type === "custom" &&
          entry?.customType === "rewind-turn" &&
          isRewindTurnData(entry.data)
        ) {
          applyBindings(
            ledger.entryToCommit,
        ...
        if (
          ...
          if (currentCommitSha)
            ledger.latestCurrentCommitSha = currentCommitSha;
  ...
  async function resolveEntrySnapshotWithLineage(
    entryId: string,
    sessionFile = currentSessionFile,
    ...
    while (cursor) {
      const ledger =
        cursor === currentSessionFile
          ? buildCurrentSessionLedgerFromMemory()
          : await parseSessionLedgerFile(cursor);
      if (!ledger) break;
  ...
  async function reconstructState(ctx: ExtensionContext) {
    ...
    if (
      activeBranchState.currentCommitSha &&
      (await commitExists(activeBranchState.currentCommitSha))
    ) {
      activeBranchState.currentTreeSha = await getCommitTreeSha(
        activeBranchState.currentCommitSha,
      );
  ...
  async function discoverSessionFiles(
    ...
    while (ancestorCursor) {
      ...
      const ledger =
        ancestorCursor === currentSessionFile
          ? buildCurrentSessionLedgerFromMemory()
          : await parseSessionLedgerFile(ancestorCursor);
  ...
  async function maybeSweepRetention(
    ctx: ExtensionContext,
    reason: "startup" | "new-snapshots" | "shutdown",
  ) {
    const retention = getRetentionSettings();
    if (!retention) return;
    if (
      reason === "new-snapshots" &&
      newSnapshotsSinceSweep < RETENTION_SWEEP_THRESHOLD
    )
      return;
    if (
      reason === "shutdown" &&
      sweepCompletedThisSession &&
      newSnapshotsSinceSweep < RETENTION_SWEEP_THRESHOLD
    )
      return;
    if (!repoRoot) return;
  ...
  async function runRetentionSweep(
    ...
    const startupBudgetMs =
      reason === "startup" ? getStartupSweepBudgetMs() : undefined;
    const startedAt = Date.now();

    const budgetExceeded = () =>
      typeof startupBudgetMs === "number" &&
    ...
    if (budgetExceeded()) {
      notify(
        ctx,
        `Rewind retention startup sweep skipped after exceeding ${startupBudgetMs}ms budget`,
        "warning",
      );
      return;
    }
    ...
    for (const ledger of ledgers) {
      ...
      for (const reference of ledger.references) {
        ...
        if (
          ...
          retention.pinLabeledEntries &&
          reference.entryId &&
          ledger.labeledEntryIds.has(reference.entryId)
        ) {
    ...
    if (
      typeof retention.maxSnapshots === "number" &&
      retention.maxSnapshots >= 0 &&
      candidates.length > retention.maxSnapshots
    ) {
    ...
    const liveSet = [
      ...new Set([
        ...pinnedCommits,
        ...candidates.map(([commitSha]) => commitSha),
      ]),
    ];
    ...
    if (reason !== "startup") {
      try {
        ...
        if (result.code !== 0) {
          throw new Error(
            result.stderr.trim() ||
              `git gc --auto failed with code ${result.code}`,
          );
        }
      } catch {
  ...
  async function checkpointEntry(ctx: ExtensionContext, entryId: string) {
    ...
    if (!isGitRepo) return;
    const entry = ctx.sessionManager.getEntry(entryId) as
      | SessionLikeEntry
      | undefined;
  ...
  async function initializeForSession(ctx: ExtensionContext) {
    ...
    maybeSweepRetention(ctx, "startup").catch((error) => {
      notify(
        ctx,
        `Rewind retention startup sweep failed: ${error instanceof Error ? error.message : String(error)}`,
        "warning",
      );
    });
  }
  ...
  pi.events.on("rewind:checkpoint-entry", (data) => {
    ...
    checkpointEntry(ctx, data.entryId).catch((error) => {
      notify(
        ctx,
        `Rewind: failed to checkpoint ${data.entryId} (${error instanceof Error ? error.message : String(error)})`,
        "warning",
      );
    });
  });
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    const previousLedger = await parseSessionLedgerFile(
  ...
  pi.on("turn_start", async (event, ctx) => {
    ...
    try {
      ...
      bindPendingPromptUser(
        ctx.sessionManager.getBranch() as SessionLikeEntry[],
        promptCollector,
      );
    } catch (error) {
      promptCollector = null;
      notify(
        ctx,
        `Rewind: failed to capture start snapshot (${error instanceof Error ? error.message : String(error)})`,
        "warning",
      );
    }
  });
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    try {
      ...
      const assistantEntry = findAssistantEntryForTurn(
        branchEntries,
        event.message,
      );
      ...
      notify(
        ctx,
        `Rewind: failed to capture assistant snapshot (${error instanceof Error ? error.message : String(error)})`,
        "warning",
      );
    }
  });
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    try {
      ...
      notify(
        ctx,
        `Rewind: failed to finalize rewind turn (${error instanceof Error ? error.message : String(error)})`,
        "warning",
      );
    } finally {
      promptCollector = null;
  ...
  pi.on("session_before_fork", async (event, ctx) => {
    ...
    try {
      ...
      const targetCommitSha = await resolveEntrySnapshotWithLineage(
        event.entryId,
      );
      const hasUndo = Boolean(
        activeBranchState.undoCommitSha &&
        (await commitExists(activeBranchState.undoCommitSha)),
      );
      ...
      if (shouldForceConversationOnly) {
        ...
        notify(
          ctx,
          `Rewind: using conversation-only fork (keep current files)${forcedBySource ? ` (${forcedBySource})` : ""}`,
        );
        return;
      }
      ...
      if (choice === "Undo last file rewind") {
        const restore = await restoreCommitExactly(
          activeBranchState.undoCommitSha!,
        );
        const nextState = {
      ...
      notify(
        ctx,
        `Rewind failed before fork: ${error instanceof Error ? error.message : String(error)}`,
        "error",
      );
      return { cancel: true };
    }
  ...
  pi.on("session_before_tree", async (event, ctx) => {
    ...
    try {
      if (
        (
          globalThis as typeof globalThis & {
            __boomerangCollapseInProgress?: boolean;
          }
        ).__boomerangCollapseInProgress === true
      ) {
        pendingTreeState = {
          currentCommitSha: await ensureSnapshotForCurrentWorktree(),
        };
      ...
      const targetEntry = ctx.sessionManager.getEntry(
        event.preparation.targetId,
      ) as SessionLikeEntry | undefined;
      const targetCommitSha = isRestorableTreeEntry(targetEntry)
        ? await resolveEntrySnapshotWithLineage(
            event.preparation.targetId,
            currentSessionFile,
          )
        : undefined;
      ...
      if (!targetCommitSha) {
        notify(
          ctx,
          "Exact file rewind is only available for user, assistant, custom message, compaction, and summary nodes",
          "error",
        );
        return { cancel: true };
      }
      ...
      notify(
        ctx,
        `Rewind failed before tree navigation: ${error instanceof Error ? error.message : String(error)}`,
        "error",
      );
      return { cancel: true };
    }
