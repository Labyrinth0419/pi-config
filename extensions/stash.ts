...
function dataDir(): string {
  ...
  return path.join(
    xdg || path.join(homedir(), ".local", "share"),
    "pi-agent-extensions",
    "stash",
  );
}
...
function loadStashFile(filePath: string): string[] {
  try {
    ...
    if (Array.isArray(data))
      return data.filter((s: unknown) => typeof s === "string");
  } catch {}
  return [];
}
...
export default function stash(pi: ExtensionAPI) {
  ...
  function updateStatus(ctx: ExtensionContext): void {
    ...
    ctx.ui.setStatus(
      "stash",
      ctx.ui.theme.fg("success", `\uf187 stash ${count}`),
    );
  }
    ...
    handler: async (ctx) => {
      ...
      if (hasText(editorText)) {
        if (sessionStashes.length >= MAX_STASHES) {
          ctx.ui.notify(
            `Session stash full (${MAX_STASHES}) — restore or delete first`,
            "warning",
          );
          return;
        }
      ...
      if (sessionStashes.length === 0) {
        const hint = globalUsed
          ? " (global stash has items — Alt+Shift+S)"
          : "";
  ...
  pi.registerShortcut("alt+shift+s", {
    description:
      "Global stash — push, apply, pop, or delete (shared across sessions)",
    handler: async (ctx) => {
      if (!ctx.hasUI) return;
      currentCtx = ctx;
  ...
  async function showGlobalPicker(ctx: ExtensionContext): Promise<void> {
    ...
    const result = await ctx.ui.custom<{
      ...
      function buildList() {
        ...
        list.onSelect = (item: any) =>
          done({ action: "pop", index: parseInt(item.value, 10) });
      ...
      return {
        render(width: number) {
          ...
          const hint = theme.fg(
            "dim",
            "Enter pop · a apply · x delete · Esc cancel",
          );
          return [
            header,
            "",
            ...list.render(width - 2).map((l: string) => ` ${l}`),
            "",
            ` ${hint}`,
          ];
        },
    ...
    if (result.action === "pop") {
      ...
      globalUsed = true;
      ctx.ui.notify(
        `Applied #${result.index + 1} from global (kept in stash)`,
        "info",
      );
  ...
  async function showPicker(
    ...
    const result = await ctx.ui.custom<{
      ...
      function buildList() {
        ...
        list.onSelect = (item: any) =>
          done({ action: "restore", index: parseInt(item.value, 10) });
        list.onCancel = () => done(null);
        return list;
      }
    ...
    if (result.action === "restore") {
      if (result.index >= 0 && result.index < stashes.length) {
        ...
        updateStatus(ctx);
        ctx.ui.notify(
          `Restored #${result.index + 1} (${scope.toLowerCase()})`,
          "info",
        );
      }
    } else if (result.action === "delete") {
      if (result.index >= 0 && result.index < stashes.length) {
        ...
        updateStatus(ctx);
        ctx.ui.notify(
          `Deleted #${result.index + 1} (${scope.toLowerCase()})`,
          "info",
        );
        if (stashes.length > 1) {
